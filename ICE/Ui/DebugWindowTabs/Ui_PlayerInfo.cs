...
namespace ICE.Ui.DebugWindowTabs
{
    internal class Ui_PlayerInfo
    {
        ...
        public static unsafe void Draw()
        {
            ...
            ImGui.InputInt("当前 XP", ref currentXp);
            ImGui.SetNextItemWidth(200);
            ImGui.InputInt("需求 XP", ref neededXp);
            ImGui.SetNextItemWidth(200);
            ImGui.InputInt("最大 XP", ref maxXp);
            ...
            ImGui.Text("需要在此处放置实际的玩家信息。它丢失了");
            ImGui.Spacing();
            ...
            ImGui.Text($"职业ID: {(uint)Player.Job}");
            ImGui.Text($"当前区域/区域ID: {Player.Territory.RowId}");
            ...
            ImGui.Text($"最佳练级任务: [{best_LevelMission}]");
        ...
        private static unsafe void ClassInfo()
        {
            ...
            foreach (var job in ManipClassInfo)
            {
                ...
                ImGui.Text($"职业ID: {job.Key} | 已解锁: {isUnlocked}");
                // 573 | Not unlocked??? 
                // 574 | Is unlocked
            ...
            {
                ImGui.Text($"职业ID: {job.Key} | 已解锁: {job.Value.HasUnlocked}");
            }
        }
