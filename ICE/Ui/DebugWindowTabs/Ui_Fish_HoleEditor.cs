...
namespace ICE.Ui.DebugWindowTabs
{
    internal class Ui_Fish_HoleEditor
    {
        ...
        public static unsafe void Draw()
        {
            ...
            {
                if (ImGui.Button("导出选中标记"))
                {
                    var exportData = ExportSingleFishingFlag(selectedZone, selectedFlag);
                    ImGui.SetClipboardText(exportData);
                    Svc.Chat.Print($"区域 {selectedZone} 在 ({selectedFlag.X}, {selectedFlag.Y}) 的钓鱼标记数据已导出到剪贴板！");
                }
            }

            ImGui.Checkbox("显示钓鱼点射线检测", ref _fishingDebug.ShowFishRay);
            ...
            {
                ImGui.TableSetupColumn("钓鱼点选择器", ImGuiTableColumnFlags.WidthFixed, 200);
                ImGui.TableSetupColumn("钓鱼点编辑器", ImGuiTableColumnFlags.WidthStretch);
                ...
                if (ImGui.BeginChild("Fishing Hole Editor", new Vector2(0, 0), true))
                {
                    if (selectedZone != 0 && selectedFlag != Vector2.Zero)
                    {
                        ...
                        if (ImGui.CollapsingHeader("该钓鱼点的所有任务"))
                        {
                        ...
                        ImGui.SameLine();
                        if (ImGui.Button("停止所有任务"))
                        {
                            P.TaskManager.Tasks.Clear();
                            P.TaskManager.Abort();
                        }
                        ImGui.Text($"可用钓鱼点：{_fishingDebug.IsFishable()}");
                        if (_fishingDebug.FindFishableLocation(out var fishablePosition))
                        {
                            ImGui.Text($"首个可用钓鱼点：{fishablePosition.Value.X:N2}, {fishablePosition.Value.Y:N2}, {fishablePosition.Value.Z:N2}");
                            if (ImGui.Button("面向钓鱼点"))
                            {
                        ...
                        ImGui.Checkbox("查看钓鱼点", ref viewAllFishingSpots);
                        ImGui.SameLine();
                        ...
                        ImGui.SetNextItemWidth(200);
                        if (ImGui.ColorEdit4("锥形颜色编辑器##ConeColorEditor", ref coneColor))
                        {
                        ...
                        ImGui.Text($"区域 {selectedZone} - X:{selectedFlag.X} Z:{selectedFlag.Y}");

                        if (ImGui.Button("添加钓鱼点"))
                        {
                        ...
                        for (int i = 0; i < fishingHole.Count; i++)
                        {
                            ...
                            if (ImGui.Selectable($"钓鱼点 {i + 1}", isSelected))
                            {
                                selectedSpotIndex = i;
                            }
                        ...
                        {
                            ImGui.Separator();
                            ImGui.Text($"正在编辑钓鱼点 {selectedSpotIndex + 1}：");
                            ...
                            ImGui.SetNextItemWidth(200);
                            if (ImGui.InputFloat3("导航位置", ref nav))
                            {
                                spot.FacePosition = nav;
                            }
                            ImGui.SameLine();
                            if (ImGui.Button("设置钓鱼朝向"))
                            {
                            ...
                            if (ImGui.SliderFloat("朝向容差（度）", ref toleranceDegrees, 1f, 45f))
                            {
                                spot.RotationTolerance = toleranceDegrees * ((float)Math.PI / 180f);
                            }

                            if (ImGui.Button("测试导航至 [新]"))
                            {
                        ...
                        using (var drawList = PctService.Draw())
                        {
                            if (viewAllFishingSpots)
                            {
                                foreach (var location in fishingHole)
                                {
                                    ...
                                    drawList.AddText(floatPos, 2667577343, $"钓鱼点 #：{holeNumber}", 1);
                                    holeNumber+= 1;
                                }
                            }
                            if (viewNavSpot)
